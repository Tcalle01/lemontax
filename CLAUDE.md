...
- `src/utils/generarArchivoIR.js` — XML/JSON generation for portal SRI en línea (Form 102) + DIMM legacy. `buildConceptosIR(params)` builds Concepto→value map (Tabla 1 Form 102). `generarXMLIR/generarJSONIR` → string content. `descargarXMLIR/descargarJSONIR(params, ruc, anio)` trigger browser download `IR_[RUC]_[anio].xml/.json`. Legacy DIMM: `generarArchivoIR/descargarArchivoIR` → `IR_DIMM_[RUC]_[anio].xml`. Only non-zero conceptos included in portal format.
- `src/utils/generarArchivoIVA.js` — XML/JSON generation for portal SRI en línea (Forms 104/104-A). `buildConceptosIVA(params)` builds Concepto→value map per SRI Tabla 1. `generarXMLIVA/generarJSONIVA` → string content. `descargarXMLIVA/descargarJSONIVA(params, ruc, periodo)` trigger browser download `IVA_[RUC]_[periodo].xml/.json`. Only non-zero conceptos are included.
- `src/supabase.js` — Supabase client init
- `src/theme.js` — Design tokens (C), catColors, catIcons, CAT_SRI, CANASTA, TIPOS_CONTRIBUYENTE, OBLIGACIONES_POR_TIPO, DIAS_VENCIMIENTO. `catColors`/`catIcons` solo contienen las 6 categorías SRI válidas + "Otros" (legacy): Salud, Educación arte y cultura, Alimentación, Vivienda, Vestimenta, Turismo. Las claves "Transporte", "Servicios", "Entretenimiento" fueron eliminadas. `CATEGORIAS` = las 6 SRI (sin "Otros"). `FacturasPage` genera sus filtros y selectores desde `Object.keys(catColors)` automáticamente.
...
- `src/pages/IvaDeclaracionPage.jsx` — **TODO 1** Declaración mensual IVA (Form 104). Aplica a: `dependencia_con_extras`, `freelancer_general`, `arrendador_general`. Carga compras (es_venta=null/false) y ventas (es_venta=true) del período, calcula IVA, modal Form 104 pre-llenado, botones "Descargar XML" y "Descargar JSON" (via `generarArchivoIVA.js`). Guarda en `declaraciones_iva` con `tipo='mensual'`, `periodo='YYYY-MM'`.
- `src/pages/IvaSemestralPage.jsx` — **TODO 2** Declaración semestral IVA (Form 104-A). Solo para `rimpe_emprendedor`. Período: S1 = enero–junio (vence julio), S2 = julio–diciembre (vence enero año siguiente). Incluye: tabla desglosada mes a mes (ventas base, IVA cobrado, compras total, IVA pagado, saldo mensual), gráfico de barras recharts (Ventas vs Compras por mes), totales consolidados, modal Form 104-A pre-llenado, botones "Descargar XML" y "Descargar JSON" (via `generarArchivoIVA.js`), marcar como presentada. Guarda en `declaraciones_iva` con `tipo='semestral'`, `periodo='YYYY-S1'` o `'YYYY-S2'`.
- `src/pages/GastosPersonalesPage.jsx` — **TODO 3** ✅ AGP completo en `/obligaciones/gastos-personales/:anio`. Dos modos según el año: **Proyección** (`anioNum >= currentYear`) y **Clasificación** (años pasados). Modo proyección — sección 1: inputs por categoría con toggle Mensual/Anual, muestra equivalente anual en tiempo real, botón "Guardar proyección" → upsert en `declaraciones_agp` como `estado='borrador'`, pre-llena desde declaracion guardada (totales ÷ 12). Modo clasificación — sección 1: facturas sin clasificar del año — 6 botones de categoría (inc. Turismo) + "No es gasto personal", clasifican en Supabase con animación fade-out. Secciones comunes: (2) resumen por categoría con totales y barra de progreso hacia `min(50% ingresos brutos, $15,817)`; (3) ahorro estimado = efectivo × 15%; (4) en proyección → nota informativa sobre presentación en feb siguiente; en clasificación → botones Formulario GP / Anexo GSP / Generar ambos + "Marcar como presentada" (modal con fecha) → upsert en `declaraciones_agp`. `esProyeccion = anioNum >= new Date().getFullYear()`. `totalesCat` = proyectados o de facturas según modo. Ingresos brutos = `salario × 12 + totalVentas` del año.
...
| Educación, arte y cultura | 5040 | campo 107 / casillero 773 | `"5040"` |
...
| Otros | — | Not deductible (internal marker only) | Not included |
...
- **AGP modo proyección vs clasificación** — `esProyeccion = anioNum >= new Date().getFullYear()`. En proyección: `totalesCat` = inputs del usuario (mensual × 12 o anual directo); toggle global Mensual/Anual; guardar upsert como `estado='borrador'`; pre-fill desde `declaraciones_agp` guardada (totales ÷ 12). En clasificación: `totalesCat` = agregado de `facturas` del año por categoría. `sinClasificar = esProyeccion ? [] : facturas.filter(...)`. Sección 4 diferente por modo: proyección → nota informativa; clasificación → GP/GSP + marcar presentada.
...
- **Categorías SRI (gastos personales)** — Solo existen 6 categorías válidas: Salud, Educación arte y cultura, Alimentación, Vivienda, Vestimenta, Turismo. "Otros" es un marcador interno (no es una categoría SRI). "Transporte", "Servicios", "Entretenimiento" no existen y fueron eliminadas. La clave del DB para educación era "Educación" (legacy) → renombrada a "Educación, arte y cultura"; para backwards compat con facturas existentes, los filtros de agregación usan `aliases = ["Educación, arte y cultura", "Educación"]`. `CATS_AGP` en DeclaracionIRPage incluye ambas claves. `sriExport.js` suma `eduTotal = catTotals["Educación, arte y cultura"] + catTotals["Educación"]`.
...
- **XML/JSON portal IR (Form 102)** — `buildConceptosIR(params)` en `src/utils/generarArchivoIR.js`. Mapeo Casillero→Concepto: ingresos_dependencia→3240 (C741), ingresos_facturacion libre ejercicio→2990 (C612), ingresos_facturacion empresarial→1280 (C611), ingresos_otros→3022, gastos libre ejercicio→3000, gastos empresariales→1290, GP salud→3290, educación→5040, alimentación→3300, vivienda→3310, vestimenta→3320, total GP→3330, base imponible→3480, IR causado general→3490, IR RIMPE emprendedor→5680+3505, IR RIMPE negocio popular→5688, retenciones dependencia→3515, retenciones fuente→3510, anticipos→3530, ingresos RIMPE→4825, gastos RIMPE→4826, IR a pagar→3562+3570 (positivo) o saldo favor→3564+3590 (negativo). Mismo formato `<detallesDeclaracion>` que IVA. `descargarXMLIR/descargarJSONIR(params, ruc, anio)` → `IR_[RUC]_[anio].xml/.json`. DIMM legacy renombrado a `IR_DIMM_*`.
- **DeclaracionIRPage resumen step** — botones "Descargar XML" + "Descargar JSON" (portal SRI) en fila; modal instrucciones muestra pasos para subir al portal SRI en línea (no DIMM). `buildParamsIR()` helper construye el objeto de parámetros desde el estado local.
- **XML/JSON portal IVA** — `generarArchivoIVA.js` implementa la Tabla 1 de la guía SRI (Casillero → Concepto). Formato: `<detallesDeclaracion><detalle concepto="450">100.00</detalle>...</detallesDeclaracion>` (XML) o `{ "detallesDeclaracion": { "450": "100.00" } }` (JSON). Conceptos clave: 450/460/470 (ventas gravadas brutas/netas/IVA), 570/580 (ventas 0%), 860/870/880 (totales ventas), 1270/1280/1290 y 1780/1790/1800 (adquisiciones brutas/netas/IVA con CT), 2130 (crédito tributario), 2140/2250/2610 (impuesto a pagar si saldo>0) o 2150/2220 (crédito a favor si saldo<0). Saldo positivo/negativo son mutuamente excluyentes. Archivo: `IVA_[RUC]_[periodo].xml` donde periodo = `YYYY-MM` (mensual) o `YYYY-S1`/`YYYY-S2` (semestral).
