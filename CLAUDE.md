...
- `src/pages/FacturasPage.jsx` — **TODO 6 ✅** Dos tabs: "Compras" (facturas importadas de Gmail, es_venta=false/null) y "Ventas" (es_venta=true). Para `dependencia_pura`: solo muestra Compras. Tab Ventas: banner resumen mensual clickeable → módulo IVA del mes (rimpe_emprendedor → iva-semestral, otros → iva mensual). Filtros mes/año. Botón "Agregar ingreso" → modal con 3 opciones: (A) Subir XML del SRI (parse via parsearXMLVenta.js, preview datos, estado cobro), (B) Formulario manual (fecha, n°factura, cliente, RUC cliente, descripción, subtotal, IVA 0%/15%, total auto, estado cobro), (C) Ingreso rápido (fecha, descripción, monto). Lista de ventas: cliente, descripción, fecha, base, IVA%, total, badge de estado (click para toggle cobrado↔pendiente). Campos nuevos requeridos en tabla facturas: `estado_cobro`, `cliente_nombre`, `cliente_ruc`, `numero_factura`, `descripcion`.
...
- **Ventas tab filtering (TODO 6)** — Compras: `.neq("es_venta", true)` (captura null y false). Ventas: `.eq("es_venta", true)`. Ventas se filtran client-side por mes/año seleccionado (carga todos una vez). `estado_cobro` se togglea inline con click en badge → update inmediato en Supabase. Para `dependencia_pura`: solo muestra tab Compras sin botón de agregar. Ventas con XML usan upsert (onConflict user_id,clave_acceso); manuales y rápidas usan insert con UUID generado (`crypto.randomUUID()`). Resumen mensual banner navega a: rimpe_emprendedor → `/obligaciones/iva-semestral/{year}/{S1|S2}`; otros con IVA → `/obligaciones/iva/{year}/{MM}`.
